...
class AgentOrchestrator:
    ...
    def print_report(self, report):
        ...
        # Configuration
        print("\n🔧 CONFIGURATION")
        silent_status = ('✅ Enabled' if report['configuration']['silent_mode']
                         else '❌ Disabled')
        print(f"  • Silent Mode: {silent_status}")
        api_status = ('✅ Configured' if report['configuration']['api_key_configured']
                      else "⚠️ Missing (workers won't run)")
        ...
        for expedition, status in report['scene_completion'].items():
            if status.get('exists', True):
                print(
                    f"  • {expedition}: {
                        status['lines']} lines ({
                        status['estimated_complete']}% complete)")
                if status['placeholders'] > 0:
                    print(
                        f"    ⚠️ {
                            status['placeholders']} placeholders remaining")

        # Conflicts
        print("\n⚠️ MERGE CONFLICTS")
        has_conflicts = any(c.get('has_conflicts', False)
                            for c in report['merge_conflicts'])
        if has_conflicts:
    ...
    def generate_recommendations(self, report):
        ...
        if not report['configuration']['api_key_configured']:
            recommendations.append(
                "🔴 CRITICAL: Add ANTHROPIC_API_KEY to repository secrets to activate workers")

        # Worker initialization
        for worker, info in report['workers'].items():
            if not info.get('exists'):
                recommendations.append(
                    f"Initialize {worker} by running its workflow manually")

        # Merge conflicts
        conflict_branches = [c['branch']
                             for c in report['merge_conflicts'] if c.get('has_conflicts')]
        if conflict_branches:
            recommendations.append(
                f"Resolve merge conflicts in: {
        ...
        if report['task_queue'].get('needs_review', 0) > 3:
            recommendations.append(
                f"{report['task_queue']['needs_review']} tasks need human review")
        ...
        for expedition, status in report['scene_completion'].items():
            if status.get('placeholders', 0) > 10:
                recommendations.append(
                    f"{expedition} has {
                        status['placeholders']} placeholders - prioritize completion")

        # Health score
        if report['health_score'] < 70:
            recommendations.append(
                "⚠️ System health below 70% - review and address issues")

        if not recommendations:
            recommendations.append(
                "✅ System healthy - workers can continue autonomous operation")
    ...
    def create_markdown_summary(self, report, filepath):
        ...
        with open(filepath, 'w') as f:
            f.write("# Agent Management Status Report\n\n")
            f.write(
            ...
            f.write(
                f"- Silent Mode: {'Enabled' if report['configuration']['silent_mode'] else 'Disabled'}\n")
            f.write(
                f"- API Key: {'Configured' if report['configuration']['api_key_configured'] else 'Missing'}\n\n")
            ...
            for worker, info in report['workers'].items():
                ...
                if info.get('exists'):
                    ...
                    f.write(
                        f"- **Last Activity:** {info.get('last_commit_time', 'unknown')}\n")
                    f.write(
                        f"- **Last Commit:** {info.get('last_commit_msg', 'unknown')}\n\n")
                else:
                    f.write("- **Status:** Not initialized\n\n")
            ...
            for expedition, status in report['scene_completion'].items():
                if status.get('exists', True):
                    ...
                    if status['placeholders'] > 0:
                        f.write(
                            f"  - ⚠️ {status['placeholders']} placeholders remaining\n")
